...
function CertificateItem ({ time, title, certificatorUrl, certificatorName, credentialUrl, img }: Certificate) {
  return (
    <li className="ms-3 p-4 rounded-xl hover:bg-neutral-300/20 dark:hover:bg-neutral-800/20  duration-100">
      <div className="flex justify-between gap-5 md:flex-row">
        ...
        <h3 className="text-lg font-semibold text-neutral-800 dark:text-neutral-200">{title}</h3>
          <a href={certificatorUrl} target="_blank" className="text-sm font-medium duration-150 text-neutral-700 dark:text-neutral-400 opacity-80 hover:opacity-100" aria-label='Link to certificator page' >
            <span>{certificatorName}</span>
          </a>

          <a href={credentialUrl} target="_blank" className="mt-1.5 inline-flex gap-1 items-center text-xs font-medium justify-center rounded-xl bg-neutral-300/60 dark:bg-neutral-800 py-2 px-3 opacity-80 transition-opacity duration-150 hover:opacity-100 group w-28 h-8" aria-label={`Link to ${title} certificate`} >
            Credential
