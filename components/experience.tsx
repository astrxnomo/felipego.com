  ...
  return (
    ...
    <ol className="relative space-y-1 border-s-2 border-neutral-300 dark:border-neutral-800">
        {experienceData.map(experience => (
            <ExperienceItem key={experience.id} time={experience.time} title={experience.title} companyUrl={experience.companyUrl} companyName={experience.companyName} description={experience.description} />
        ))}
    </ol>
    </section>
  )
...
function ExperienceItem ({ time, title, companyUrl, companyName, description }: ExperienceItemProps) {
  return (
    <li className="ms-3 p-4 rounded-xl hover:bg-neutral-300/20 dark:hover:bg-neutral-800/20 duration-100">
      ...
      <div className="mt-2 flex flex-col space-y-0.5">
        ...
        <a href={companyUrl} target="_blank" className="text-sm font-medium duration-150 text-neutral-700 dark:text-neutral-400 opacity-80 hover:opacity-100">
          <span>{companyName}</span>
        </a>
      </div>
      <p className="mt-3 text-pretty text-sm text-neutral-600 dark:text-neutral-400">{description}</p>
    </li>
  )
}
